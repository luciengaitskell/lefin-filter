...
async def axis_comparator_test(dut):
    ...
    for i in range(NUM_TESTS):
        ...
        packed_input = (input_a & ((1 << INPUT_ELT_WIDTH) - 1)) << INPUT_ELT_WIDTH | (
            input_b & ((1 << INPUT_ELT_WIDTH) - 1)
        )
        ...
        if input_a >= input_b:
            assert dut.output_valid.value == 1, (
                f"Test {i}: Expected output_valid to be 1 but got 0 for inputs {input_a}, {input_b}"
            )
            assert dut.top_half_greater.value == 1, (
                f"Test {i}: Expected top_half_greater to be 1 but got 0 for inputs {input_a}, {input_b}"
            )
        else:
            assert dut.output_valid.value == 1, (
                f"Test {i}: Expected output_valid to be 1 but got 0 for inputs {input_a}, {input_b}"
            )
            assert dut.top_half_greater.value == 0, (
                f"Test {i}: Expected top_half_greater to be 0 but got 1 for inputs {input_a}, {input_b}"
            )
...
async def axis_comparator_test_with_missing_tlast(dut):
    ...
    for i in range(NUM_TESTS):
        ...
        assert dut.output_valid.value == 0, (
            f"Test {i}: Expected output_valid to be 0 due to missing tlast for inputs {input_a}, {input_b}"
        )
...
if __name__ == "__main__":
    build_and_run_sim(
        ...
        parameters={"DATA_WIDTH": INPUT_ELT_WIDTH},
    )
