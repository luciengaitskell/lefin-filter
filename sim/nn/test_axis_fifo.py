...
async def test_fifos_basic(dut):
    ...
    tb.ind.append({"type": "write_burst", "contents": {"data": data_in}})
...
async def test_fifos_basic2(dut):
    ...
    await ClockCycles(
        dut.aclk, NUM_ITER // 2
    )  # wait a bit before starting but before a tlast is sent
    dut.packet_input_valid.value = 1
    dut.packet_input_good.value = 1
    await ClockCycles(dut.aclk, 1)
    dut.packet_input_valid.value = 0
...
async def test_fifos_basic3(dut):
    ...
    for _ in range(NUM_ITER):
        ...
        tb.ind.append({"type": "write_single", "contents": {"data": data_in}})
    tb.ind.append({"type": "write_single", "contents": {"data": 0x55555555, "last": 1}})
...
async def test_fifos_backpressure(dut):
    ...
    for i in range(NUM_ITER):
        ...
        if i == NUM_ITER - 1:
            tb.ind.append(
                {"type": "write_single", "contents": {"data": data_in, "last": 1}}
            )
        else:
            tb.ind.append(
                {"type": "write_single", "contents": {"data": data_in, "last": 0}}
            )
...
async def test_fifos_drop_packet(dut):
    ...
    assert len(tb.data_in) == NUM_ITER, (
        f"Should have recieved all input data, but got: {len(tb.data_in)} instead"
    )
    assert len(tb.data_out) == 0, (
        f"Should have recieved no output data, but got: {len(tb.data_out)} instead"
    )
...
async def test_fifos_drop_packet_with_good(dut):
    ...
    tb.ind.append({"type": "write_burst", "contents": {"data": data_in1}})
    # pause for a bit
    tb.ind.append({"type": "pause", "duration": 3})
    # good packet after
    data_in2 = [0x22222222 for _ in range(NUM_ITER)]
    tb.ind.append({"type": "write_burst", "contents": {"data": data_in2}})
    # pause for a bit
    tb.ind.append({"type": "pause", "duration": 3})
    ...
    assert len(tb.data_in) == NUM_ITER * 2, (
        f"Should have recieved all input data, but got: {len(tb.data_in)} instead"
    )
    assert len(tb.data_out) == NUM_ITER, (
        f"Should have recieved only good output data, but got: {len(tb.data_out)} instead"
    )
    for data in tb.data_out:
        assert data["data"] == 0x22222222, (
            f"Output data should only contain good packet data, got: {data} instead"
        )
...
async def test_fifos_sandwich_drop(dut):
    ...
    tb.ind.append({"type": "write_burst", "contents": {"data": data_in3}})
    # pause for a bit
    tb.ind.append({"type": "pause", "duration": 3})
    ...
    assert len(tb.data_in) == NUM_ITER * 3, (
        f"Should have recieved all input data, but got: {len(tb.data_in)} instead"
    )
    assert len(tb.data_out) == NUM_ITER * 2, (
        f"Should have recieved only good output data, but got: {len(tb.data_out)} instead"
    )
    for i, data in enumerate(tb.data_out):
        assert data["data"] == expected_output[i], (
            f"Output data should only contain good packet data, got: {data} instead"
        )
