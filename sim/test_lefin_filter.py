...
class LefinFilterTestbench(ModelTestbench):
    def __init__(self, dut, **kwargs):
        super().__init__(dut, **kwargs)
        self.scoreboard_queue: deque[int]
    ...
    def compare_fn(self, raw_result):
        ...
        result_okay = raw_result == expected_result
        if not result_okay:
            self.scoreboard.errors += 1

            self.dut._log.error(
                f"Mismatch! Got {raw_result}, expected {expected_result}"
            )
        else:
            self.dut._log.info(f"Match! Got {raw_result}, expected {expected_result}")
        return result_okay
    ...
    def send_test_input(self):
        ...
        if not preds.item():
            self.expected_data_out.append(packed_chunks)
            self.scoreboard_queue.extend(packed_chunks)
            return len(packed_chunks)
...
async def test_a(dut):
    """cocotb test for AXI-Stream full design"""
    tb = LefinFilterTestbench(dut)
    ...
    for i in range(NUM_ITER):
        expected_outputs = tb.send_test_input()
        if random.randint(0, 2):
            tb.ind.append({"type": "pause", "duration": random.randint(1, 6)})

        tb.outd.append({"type": "read", "duration": expected_outputs})
