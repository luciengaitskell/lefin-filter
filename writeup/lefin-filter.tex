...
The IEEE 802.3 standard defines Ethernet, a technology connecting devices everywhere today. Subsequent revisions and additional clauses have pushed bitrates higher and higher, with consumer devices now supporting bitrates up to 2.5 gigabits per second and modern data center links supporting speeds in excess of 10 Gbps per channel. At these speeds a mid-sized Ethernet frame (791 bytes) can be sent in about 640 nanoseconds.
...
Network Interface Card (NIC) to copy the packet into
the system memory of the network filter server.
This requires the packet traversing the NIC,
...
first byte can be processed \cite{paper-ix-os, paper-neugebauer}, with a similar latency on the outbound path. With some businesses measuring server latency in single-digit microseconds, the downsides outweigh the benefits of a system like this.
...
\begin{itemize}
    ...
    \item Provide latency under $3\,\mu\text{s}$
    \item Achieve at least 95\% classification and filter accuracy
\end{itemize}

\subsection{Anticipated Challenges}
As with most machine learning models, demonstrated architectures, although small, rely on numerous floating-point multiplies and accumulates (MACs). The model proposed by Wang et al.\cite{paper-wang-conv1d} would require in excess of $40\,\text{Mb}$ of weights, which would require a large FPGA and difficult routing to accomodate. Additionally, Ethernet is an intricate standard and imposes specific timing and ordering which our design must respect. Finally, the line rate of modern Ethernet is extremely fast, and producing a system that can process every byte quickly and accurately is challenging.
...
With the addition of a QSFP port, this enables the use of the AMD 10G/25G High Speed Ethernet Subsystem IP \cite{ref-eth-sub}
providing an intuitive way to connect our filter
...
we limit this purging to three cycles,
...
This process is described in Fig. \ref{fig:mod-inter-fsm}.
...
and outputs, the testbench harness has been generalized and shared
between the tests.
...
and assigned in the model HDL design.
...
\end{figure}
We set up a test environment to evaluate our design on the RFSoC 4x2.
...
\subsection{Dataset}
We ran our real-world demonstration and evaluation using the
...
To evaluate accuracy, we sent Ethernet frames from the
validation split of the dataset from one network interface
and recorded the packets received on the other interface.
This allowed the evaluation script to determine the accuracy
in terms of dropped malicious frames and benign packets
that are successfully passed through.
The test script was limited to sending at most $\sim800$ packets
...
We were able to use the \href{https://github.com/torvalds/linux/blob/d358e5254674b70f34c847715ca509e46eb81e6f/tools/testing/selftests/net/timestamping.c}
...
The results shown in Table \ref{tab:results-latency}
show that we are unable to measure the latency with our current setup.
Additionally, it indicates that the timestamping script and/or
\texttt{tcpdump} is likely not using HW timestamps.

By our estimates, even for sub-$784\,$B Ethernet frames,
the passthrough will be delayed by:
\begin{enumerate}
    \item 3 additional cycles for padding
    \item 2 cycles to go through 1D convolution
    \item 1 cycle to finish global max pool
    \item 0 cycles to go through fully connected
    \item 1 cycle to go through Comparator
    \item 1 cycle for the FIFO to begin reading
\end{enumerate}
Therefore, we estimate, given the $6.4\,$ns clock period,
an additional latency of around $50\,\text{ns}$.
This would be in addition to any latency introduced by the
...
The FPGA resource utilization, shown in Table \ref{tab:imp_rep},
...
In scenarios with back-to-back frame transfers from the Ethernet subsystem, our drop buffer FIFO may cause a backlog if it needs to finish propagating a large packet. Although we were far from reaching this limit and unable to observe it with our demonstration platform, we believe it could be beneficial to proactively address this. To minimize the chances of this backlog, we can implement a variety of solutions ranging from duplicating the FIFO and adding an arbiter for the active FIFO, creating a FIFO which holds and marks more frames, or implementing cut-through with FCS corruption to minimize delay.

\subsection{More Types of Classifications}
Malware is ever-evolving. While the current model architecture is sufficient for identifying malware vs. benign frames, expanding our layers and adding a softmax layer may allow us to classify certain types of malware or types of benign frames.
...
\begin{thebibliography}{00}
    ...
    \bibitem{ref-eth-sub}AMD 10G/25G High Speed Ethernet Subsystem https://docs.amd.com/r/en-US/pg210-25g-ethernet/Introduction
